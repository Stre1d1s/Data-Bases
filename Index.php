...
<body>
  <nav class="navbar navbar-expand-lg bg-body-tertiary">
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav">
          <li class="nav-item">
            <a class="nav-link active" aria-current="page" href="index.php">Erwtima 1</a>
          </li>
          <li class="nav-item">
  ...
  <?php
  $stmt = $conn->prepare("SELECT M.man_id AS 'ID Διευθυντή', 
    M.monomateponimo AS 'Ονοματεπώνυμο Διευθυντή', 
    SUM(MI.poso) AS 'Συνολικό Ποσό Πληρωμών' 
    FROM PLIRONI P JOIN MANAGER M ON P.man_id = M.man_id 
    JOIN MISTHOS MI ON P.plir_id = MI.plir_id 
    GROUP BY M.man_id, M.monomateponimo; ");
  $stmt->execute(); //ektelesi tou erwtimatos
  $result = $stmt->get_result(); //anaktisi apotelesmatos 

  ?>

  <h1 style="margin-left:20%">Εμφάνιση συνολικόυ ποσού που κάθε διευθυντή έχει πληρώσει τους υπαλλήλους του</h1>
  ...
  <table class="table" id="mytable">
    ...
    <tbody>
      ...
      while ($row = $result->fetch_assoc()) { //gia kathe grammi tou select
        echo '<tr>';
        echo '<td>' . $row['ID Διευθυντή'] . '</td>';
        echo '<td>' . $row['Ονοματεπώνυμο Διευθυντή'] . '</td>';
        echo '<td>' . $row['Συνολικό Ποσό Πληρωμών'] . '</td>';
        echo '<tr>';
      }
      ?>
