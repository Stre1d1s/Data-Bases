...
<body>
    ...
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav">
          ...
          <li class="nav-item">
            <a class="nav-link active" aria-current="page" href="erwtima4.php">Erwtima 4</a>
          </li>
          <li class="nav-item">
  ...
  $stmt = $conn->prepare("SELECT  
    AVG(A.rating) AS 'Μέσος Όρος Βαθμολογίας' 
    FROM AKSIOLOGISI A 
    JOIN PERIGRAFI P ON A.rep_id = P.rep_id 
    JOIN ERGAZOMENOS E ON P.erg_id = E.erg_id; ");
  $stmt->execute(); //ektelesi tou erwtimatos
  $result = $stmt->get_result(); //anaktisi apotelesmatos 

  ?>

<h1 style="margin-left:20%">Εμφάνιση μέσου όρου αξιολόγησης</h1>

  <table class="table" id="mytable">
    <thead>
      <tr>
        <th scope="col">Μέσος Όρος Βαθμολογίας</th>
      </tr>
    </thead>
    <tbody>
      ...
      while ($row = $result->fetch_assoc()) { //gia kathe grammi tou select
        echo '<tr>';
        echo '<td>' . $row['Μέσος Όρος Βαθμολογίας'] . '</td>';
        echo '<tr>';
      }
      ?>
