...
<body>
    ...
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav">
          ...
          <li class="nav-item">
            <a class="nav-link active" aria-current="page" href="erwtima8.php">Erwtima 8</a>
          </li>
          <li class="nav-item">
  ...
  $stmt = $conn->prepare("SELECT E.eonomateponimo AS 'Ονοματεπώνυμο Εργαζόμενου', 
    M.monomateponimo AS 'Ονοματεπώνυμο Διευθυντή', A.rating AS 'Βαθμολογία' 
    FROM ERGAZOMENOS E 
    INNER JOIN PERIGRAFI P ON E.erg_id = P.erg_id 
    INNER JOIN MANAGER M ON P.man_id = M.man_id 
    INNER JOIN AKSIOLOGISI A ON P.rep_id = A.rep_id; ");
  $stmt->execute(); //ektelesi tou erwtimatos
  $result = $stmt->get_result(); //anaktisi apotelesmatos 

  ?>

<h1 style="margin-left:20%">Εμφάνιση των υπαλλήλων, των διευθυντών και των βαθμολογιών αξιολόγησης που τους έχουν δοθεί</h1>

  <table class="table" id="mytable">
    <thead>
      <tr>
        <th scope="col">Ονοματεπώνυμο Εργαζόμενου</th>
        <th scope="col">Ονοματεπώνυμο Διευθυντή</th>
    ...
    <tbody>
      ...
      while ($row = $result->fetch_assoc()) { //gia kathe grammi tou select
        ...
        echo '<td>' . $row['Ονοματεπώνυμο Διευθυντή'] . '</td>';
        echo '<td>' . $row['Βαθμολογία'] . '</td>';
        echo '<tr>';
      }
      ?>
